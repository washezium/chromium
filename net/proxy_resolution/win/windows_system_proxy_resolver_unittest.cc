...
class WindowsSystemProxyResolverTest : public TestWithTaskEnvironment {
 public:
  ...
  void TearDown() override {
    EXPECT_TRUE(proxy_resolver_->HasOneRef()) << "This test has a memory leak!";
  ...
  void DoProxyConfigTest(const ProxyConfig& proxy_config) {
    ...
    if (proxy_config.has_pac_url())
      pac_url.assign(proxy_config.pac_url().spec().begin(),
                     proxy_config.pac_url().spec().end());

    base::string16 proxy;
    if (!proxy_config.proxy_rules().single_proxies.IsEmpty())
      proxy.assign(
          proxy_config.proxy_rules().single_proxies.ToPacString().begin(),
          proxy_config.proxy_rules().single_proxies.ToPacString().end());

    base::string16 proxy_bypass;
    if (!proxy_config.proxy_rules().bypass_rules.ToString().empty())
      proxy_bypass.assign(
          proxy_config.proxy_rules().bypass_rules.ToString().begin(),
          proxy_config.proxy_rules().bypass_rules.ToString().end());
