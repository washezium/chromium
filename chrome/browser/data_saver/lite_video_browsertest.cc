...
// Fails occasionally on ChromeOS. http://crbug.com/1111570
...
IN_PROC_BROWSER_TEST_F(LiteVideoBrowserTest,
                       DISABLE_ON_CHROMEOS(SimplePlayback)) {
  ...
  RetryForHistogramUntilCountReached(histogram_tester(),
                                     "LiteVideo.URLLoader.ThrottleLatency", 4);
}

IN_PROC_BROWSER_TEST_F(LiteVideoBrowserTest,
                       DISABLE_ON_CHROMEOS(SimplePlaybackWithSubframe)) {
  ...
  RetryForHistogramUntilCountReached(histogram_tester(),
                                     "LiteVideo.URLLoader.ThrottleLatency", 8);
}
