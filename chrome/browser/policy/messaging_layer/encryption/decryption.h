...
class Decryptor : public base::RefCountedThreadSafe<Decryptor> {
 public:
  ...
  class Handle {
   public:
    Handle(base::StringPiece symmetric_key, scoped_refptr<Decryptor> decryptor);
  ...
  // Recreates symmetric key with local private key and peer public value and
  // returns it or error status.
  StatusOr<std::string> DecryptKey(base::StringPiece public_key,
