...
void BrowserAccessibilityManagerWin::HandleTextChangedEvent(
    BrowserAccessibility* node) {
  DCHECK(in_on_accessibility_events_);
  text_changed_events_.insert(GetUiaTextPatternProvider(node));
}

void BrowserAccessibilityManagerWin::HandleTextSelectionChangedEvent(
    BrowserAccessibility* node) {
  DCHECK(in_on_accessibility_events_);
  text_selection_changed_events_.insert(GetUiaTextPatternProvider(node));
}
...
void BrowserAccessibilityManagerWin::FinalizeAccessibilityEvents() {
  ...
  for (auto&& event_node : text_selection_changed_events_)
  ...
  for (auto&& event_node : text_changed_events_)
